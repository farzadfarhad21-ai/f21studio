...
const DESIGN_COLORS = ["#a855f7", "#fafafa", "#c084fc", "#0a0a0a"];

const CODE_SOURCE =
  'const brand = F21Studio.create({\n  website:    "deployed ✓",\n  visuals:    "generated ✓",\n  automation: "running ✓",\n})\n// → Ready to launch.';

const METRICS = [
  { target: 12400, format: (n: number) => n.toLocaleString() + "+", label: "Monthly Visitors" },
  { target: 42,    format: (n: number) => (n / 10).toFixed(1) + "%", label: "Avg. Conversion Rate" },
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        ...
        <span className="text-[#a3a3a3] text-xs font-mono">AI Processing...</span>
      </div>
      <div className="space-y-2">
        {AI_LINES.map(([key, val], i) => (
          <div
            key={key}
          ...
          >
            <span className="text-[#a3a3a3] w-24 shrink-0">{key}</span>
...
function DesignCard({ active }: { active: boolean }) {
  ...
  return (
    <div className="space-y-5">
      ...
      <div style={{ opacity: active ? 1 : 0, transition: "opacity 0.5s ease 0.4s" }}>
        <div className="text-[#fafafa] text-5xl font-black leading-none">Aa</div>
        <div className="text-[#a3a3a3] text-xs mt-1 tracking-wide">Primary Typeface</div>
      </div>
      ...
      <div className="flex gap-2">
        {[0, 1, 2].map(i => (
          <div
            key={i}
            className="h-7 rounded-full px-3 bg-[#a855f7]/20 border border-[#a855f7]/40 flex-1"
            style={{
...
function MetricsCard({ active }: { active: boolean }) {
  ...
  return (
    <div className="grid grid-cols-3 gap-4">
      {METRICS.map((m, i) => (
        <div key={m.label}>
          <div className="text-2xl font-black text-[#fafafa] leading-none tabular-nums">
            {active ? m.format(counts[i]) : "0"}
          </div>
          <div className="text-[#a3a3a3] text-[11px] mt-1 leading-tight">{m.label}</div>
        </div>
      ))}
    </div>
  );
      ...
      <div className="max-w-6xl mx-auto space-y-28">
        {STEPS.map((step, i) => (
          ...
          >
            ...
            <div className="bg-[#111111] border border-[#262626] rounded-2xl p-6 min-h-[200px] flex flex-col justify-center">
              {cards[i]}
            </div>
          </div>
        ))}
